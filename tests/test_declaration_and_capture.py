...
# --- disclosure, and every declaration this repository ships ----------------

def test_a_plain_signature_discloses_nothing_and_is_still_reviewed() -> None:
    """The control for the whole disclosure idea.

    If `disclosure` answered truthily for an ordinary name, every shipped
    declaration would pass the test below for the wrong reason.
    """
    engagement = declaration.load(GOOD)
    assert engagement.reviewed is True
    assert engagement.disclosure is None


@pytest.mark.parametrize("marker,expected", [
    ("FIXTURE -- invented in full", "FIXTURE"),
    ("DERIVED FROM A PUBLIC DATASET", "DERIVED"),
    ("DERIVED -- not signed by anybody", "DERIVED"),
    ("NOT SIGNED -- generated by a script", "NOT SIGNED"),
    ("SYNTHETIC: generated", "SYNTHETIC"),
    # The near misses. A marker is an UPPER-CASE word at the front, not a word
    # anywhere and not any capitalisation of one.
    ("a name who derived it from a fixture", None),
    ("Fixtures Ltd, a consultancy", None),
    # THE ONE THAT USED TO BE A FALSE POSITIVE, and the reason the rule now requires
    # upper case. A real firm whose name begins with a marker word was reported as
    # NOT SIGNED, disclosed as FIXTURE -- a genuine signature read as nobody having
    # signed, which is this property's own failure mode pointing backwards. The
    # near-miss above was `Fixtures` plural, one letter away from catching it.
    ("Fixture Consulting Ltd", None),
    ("Derived Analytics LLP", None),
    ("derived by Jane Smith from the signed statement of work", None),
    ("unsigned as yet, pending the partner's review", None),
])
def test_a_disclosure_is_an_upper_case_marker_at_the_front_and_nothing_else(
        marker, expected) -> None:
    engagement = declaration.load({**GOOD, "reviewed_by": marker})
    assert engagement.disclosure == expected
    assert engagement.reviewed is True, (
        "a disclosed declaration stays admitted; this convention reports, it does "
        "not gate")


def test_every_declaration_this_repository_ships_is_honest_about_its_signature() -> None:
    """No shipped declaration may name a real party for an act it did not perform.

    The defect: the Astropy declaration's `reviewed_by` named the real Astropy SPOC
    and Finance Committee, with the funding call's nominal selection date in
    `reviewed_on`. That committee selected funding requests; it never read a JSON
    document generated by a script in this repository, and `reviewed` is
    `bool(who) and bool(when)`, so nothing downstream could tell. The explanation
    existed only as a comment in the fetch script.

    Asserted over the whole set rather than over that one file, because the next
    corpus somebody adds is the one this has to catch. The set is counted first:
    `all()` over an empty glob is True, and would make this pass by finding nothing.
    """
    import json
    import pathlib

    root = pathlib.Path(__file__).resolve().parent.parent
    shipped = sorted(root.glob("evidence/*declaration*.json")) + \
        sorted(root.glob("examples/*fixture*.json"))
    assert len(shipped) >= 3, (
        f"expected every shipped declaration, found {[p.name for p in shipped]}")

    for path in shipped:
        engagement = declaration.load(json.loads(path.read_text(encoding="utf-8")))
        assert engagement.disclosure, (
            f"{path.name} signs as {engagement.reviewed_by!r}, which reads as a "
            f"person's signature. Every declaration in this repository is a fixture "
            f"or a derivation, so each has to say so in the field a reader sees")
